...
//------------------------------------------------------------------------------------------------
   ...
   function calculateRewardToStableCoin(uint256 rewardAmount) private view returns (uint256){ //don't know why it has to be view
       ...
       RewardToStableCoinMicroether = ((rewardAmount * CrownPriceIn100Scale * stableCoinPriceIn100Scale) / 10000) / (10**18) *(10**decimalStableCoins); //convert from 100scale two times
   ...
   {    require(block.timestamp >= endDate  && startDate >0, "please wait until the reward removal date.");
        ...
        //   rewards[stakeholder] = reward;  old code
           ...
           if(rewardStableCoin > 0) transferFromContractStableCoin(stakeholder,rewardStableCoin); // this func return wei so need web3 to call this and convert to ETH 
           ...
           //removeStake(stakes[stakeholder]); //CAN't use this one cause this one invokes msg.sender not stakeholder
