...
contract CrownToken is IERC20, IERC20Metadata {
    ...
    */
    ...
    function balanceOfStableCoin(address addressOfStableCoin, address tokenOwner) public view returns(uint) {
    abstractStableCoin myStableCoin = abstractStableCoin(addressOfStableCoin);
    ...
    function transferFromContractStableCoin(address addressOfStableCoin,address receiver,uint256 numStableCoin) internal onlyOwner returns (bool) { 
        require(numStableCoin <= balanceOfStableCoin(addressOfStableCoin,address(this)), "not enough stable coin in owner's balances");
        //sender = msg.sender;
        // balancesUSDT[address(this)] -= numUSDT; use the balanceofUSDT directly
        // balancesUSDT[msg.sender] += numUSDT; 
        transferStableCoin(addressOfStableCoin, receiver,numStableCoin);
    ...
    function transferStableCoin(address addressOfStableCoin, address receiver,uint256 numTokens) public validAddress(receiver) returns(bool) {
    abstractStableCoin myStableCoin = abstractStableCoin(addressOfStableCoin);
...
//------------------------------------------------------------------------------------------------
   ...
   function calculateRewardToStableCoin(uint256 rewardAmount) private view returns (uint256){ //don't know why it has to be view
       ...
       RewardToStableCoinMicroether = ((rewardAmount * CrownPriceIn100Scale * stableCoinPriceIn100Scale) / 10000)/(10**12); //convert from 100scale two times
   ...
   function distributeRewards(address addressOfStableCoin) public onlyOwner
   {
        for (uint256 s = 0; s < stakeholders.length; s += 1){
           address stakeholder = stakeholders[s];
        ...
        //   rewards[stakeholder] = reward;  old code
           ...
           if(rewardStableCoin > 0) transferFromContractStableCoin(addressOfStableCoin,stakeholder,rewardStableCoin); // this func return wei so need web3 to call this and convert to ETH 
   ...
   function withdrawReward(address addressOfStableCoin ) public //
   {   require(block.timestamp >= endDate , "please wait until the reward removal date.");
       uint256 reward = rewards[msg.sender];
       
       uint256 rewardStableCoin = calculateRewardToStableCoin(reward); 
       
       if(rewardStableCoin > 0) transferFromContractStableCoin(addressOfStableCoin,msg.sender,rewardStableCoin); 
